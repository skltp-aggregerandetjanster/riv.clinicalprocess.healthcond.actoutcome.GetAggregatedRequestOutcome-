...
public class GetAggregatedRequestOutcomeIntegrationTest extends AbstractAggregateIntegrationTest {

	public GetAggregatedRequestOutcomeIntegrationTest() {
		super(rb.getString("TAK_TJANSTEKONTRAKT"));
	}
	
	private static final Logger log = LoggerFactory.getLogger(GetAggregatedRequestOutcomeIntegrationTest.class);

    private static final RecursiveResourceBundle rb = new RecursiveResourceBundle("GetAggregatedRequestOutcome-config");
	private static final String SKLTP_HSA_ID = rb.getString("SKLTP_HSA_ID");

    private static final String LOGICAL_ADDRESS = "logical-address";
	private static final String EXPECTED_ERR_TIMEOUT_MSG = "Read timed out";
	private static final String EXPECTED_ERR_INVALID_ID_MSG = "Invalid Id: " + TEST_RR_ID_FAULT_INVALID_ID;
	private static final String DEFAULT_SERVICE_ADDRESS = GetAggregatedRequestOutcomeMuleServer.getAddress("SERVICE_INBOUND_URL");

	protected String getConfigResources() {
		return
			"soitoolkit-mule-jms-connector-activemq-embedded.xml," +
	  		"GetAggregatedRequestOutcome-common.xml," +
//          Only load GetAggregatedRequestOutcome-common.xml, it will import the other config files since mule-deploy.properties can't load config-files from jar-files on the classpath, e.g. agp-core.jar
...
//	        "aggregating-service.xml," +
	...
	private List<ProcessingStatusRecordType> doTest(String registeredResidentId, String senderId, String originalConsumerHsaId, String correlationId, int expectedProcessingStatusSize, TestData... testData) {
		...
		GetAggregatedRequestOutcomeTestConsumer consumer = new GetAggregatedRequestOutcomeTestConsumer(DEFAULT_SERVICE_ADDRESS, senderId, originalConsumerHsaId, correlationId);
    	...
    	GetRequestOutcomeResponseType response = responseHolder.value;
    	int expextedResponseSize = testData.length;

		assertEquals(expextedResponseSize, response.getRequestOutcome().size());

		for (int i = 0; i < testData.length; i++) {
			final RequestOutcomeType responseElement = response.getRequestOutcome().get(i);
			assertEquals(registeredResidentId, responseElement.getBody().getPatient().getPersonId().getExtension());
		}
